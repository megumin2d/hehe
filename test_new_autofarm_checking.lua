    ...
    function autofarm2()
        while _G.autofarm2 == true do
            if not initialActionDone then
                -- Выполняем начальные действия только один раз за вызов функции
            ...
            if player and player.PlayerGui then
                ...
                if activeSounds then
                    ...
                    if notificSound then
                        -- Если звук Notification найден, выполняем дополнительный Charge
                        print("yes")
                        local A_1 = {}
                        local Event = game:GetService("ReplicatedStorage").Events.Tools.Charge
                        Event:FireServer(A_1)
                        ...
                        print("Звук 'Notification' не найден.")
                    end
                else
        ...
        -- Сбрасываем флаг после завершения функции
        initialActionDone = false
    end
