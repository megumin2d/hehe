...
---function Crystal zone
    local initialActionDone = false  -- Флаг для отслеживания выполнения начальных действий
    local notificationHandled = false  -- Флаг для отслеживания обработки Notification
    
    function autofarm2()
        while _G.autofarm2 == true do
            -- Выполняем начальные действия только один раз при старте функции
            if not initialActionDone then
                ...
                initialActionDone = true  -- Устанавливаем флаг в true после выполнения начальных действий
            end
            
            -- Проверяем наличие ActiveSounds и звука Notification
            local player = game:GetService("Players").LocalPlayer
            if player and player.PlayerGui then
                local activeSounds = player.PlayerGui:FindFirstChild("ActiveSounds")
                if activeSounds then
                    local notificSound = activeSounds:FindFirstChild("Notification")
                    if notificSound then
                        -- Если звук Notification найден и еще не обработан, выполняем Charge
                        if not notificationHandled then
                            local A_1 = {}
                            local Event = game:GetService("ReplicatedStorage").Events.Tools.Charge
                            Event:FireServer(A_1)
                            print("Дополнительный Charge выполнен: звук 'Notification' найден.")
                            
                            notificationHandled = true  -- Устанавливаем флаг в true после выполнения Charge
                        end
                        
                        -- Сбрасываем флаги, если звук Notification существует
                        wait(0.35)
                        print("abonga")
                        initialActionDone = false
                        notificationHandled = false
                    else
                        -- Если звук Notification отсутствует, ничего не делаем
                    end
                else
                    print("Папка 'ActiveSounds' не найдена.")
                end
            else
                print("PlayerGui не найден.")
            end
            
            -- Добавляем задержку, чтобы избежать перегрузки
            wait(0.2)
        end
    end
